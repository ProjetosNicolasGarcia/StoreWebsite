...
<x-layout>
    ...
    <div class="container mx-auto px-4 py-8 pt-24 md:pt-32" x-data="productSelector({{ json_encode($alpineConfig) }})">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16 items-start max-w-7xl mx-auto">
            
            {{-- COLUNA 1: IMAGENS PRINCIPAIS DO PRODUTO --}}
            <div class="space-y-4" aria-label="Galeria de imagens do produto" role="region">
                <div class="relative w-full group border border-gray-200 bg-white overflow-hidden" style="aspect-ratio: 1 / 1;">
                    ...
                    {{-- ✏️ Correção: Botões sempre visíveis na galeria principal --}}
                    <button x-show="galleryImages.length > 1" @click="prevImage()" aria-label="Ver imagem anterior" class="absolute left-4 top-1/2 -translate-y-1/2 bg-white hover:bg-black hover:text-white text-black p-3 rounded-none shadow-md border border-gray-200 transition-colors z-10 cursor-pointer">
                    ...
                    <button x-show="galleryImages.length > 1" @click="nextImage()" aria-label="Ver próxima imagem" class="absolute right-4 top-1/2 -translate-y-1/2 bg-white hover:bg-black hover:text-white text-black p-3 rounded-none shadow-md border border-gray-200 transition-colors z-10 cursor-pointer">
            ...
            <div class="flex flex-col space-y-6">
                <div>
                    ...
                    <div class="flex items-center space-x-2">
                        <div class="flex scale-75 origin-left" aria-hidden="true">
                            @for($i = 1; $i <= 5; $i++)
                                <svg class="w-5 h-5 {{ $reviewsCount > 0 && $i <= round($avgRating) ? 'text-black' : 'text-gray-200' }}" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                </svg>
                            @endfor
                        </div>
                        <span class="text-xs text-gray-500 font-bold uppercase tracking-widest" aria-label="{{ $reviewsCount }} avaliações de clientes">
                        ...
                        <div>
                            ...
                            <template x-if="promoText">
                                <p class="text-[10px] text-red-600 font-bold mt-2 flex items-center gap-1.5 pt-2 uppercase tracking-wide">
                                    <svg aria-hidden="true" class="w-3.5 h-3.5 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" /></svg>
                ...
                <div class="pt-4 max-w-sm space-y-4">
                    <div x-data="{ 
                            zipCode: '', loading: false, result: null, error: null, 
                            async calculate() { 
                                const cleanCep = this.zipCode.replace(/\D/g, ''); 
                                if (cleanCep.length !== 8) { this.error = 'CEP Inválido (digite 8 números)'; return; } 
                                this.loading = true; this.error = null; this.result = null; 
                                try { 
                                    const response = await axios.post('{{ route('shipping.calculate') }}', { zip_code: cleanCep, product_id: {{ $product->id }}, _token: '{{ csrf_token() }}' }); 
                                    this.result = response.data; 
                                } catch (e) { this.error = e.response?.data?.error || 'Erro ao calcular o frete.'; } 
                        ...
                        <div class="flex gap-2 mb-2" aria-label="Calcular frete e prazo de entrega">
                            ...
                            <button @click="calculate()" aria-label="Calcular opções de frete" :disabled="loading" class="h-12 px-6 border border-black rounded-none bg-black text-white text-xs font-bold uppercase tracking-widest hover:bg-white hover:text-black transition-all disabled:opacity-50 flex items-center justify-center min-w-[100px] cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black">
                                ...
                                <span x-show="loading" style="display: none;" aria-hidden="true"><svg class="animate-spin h-5 w-5 text-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></span>
                            </button>
                        </div>
                        <div aria-live="polite">
                            <div x-show="error" style="display: none;" class="text-xs font-bold text-red-600 mt-2 flex items-center gap-1"><svg aria-hidden="true" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg><span x-text="error"></span></div>
                            <div x-show="result" style="display: none;" class="mt-4 space-y-3 bg-gray-50 border border-gray-200 p-4" aria-label="Opções de entrega disponíveis">
                                <template x-for="option in result" :key="option.name">
                                    <div class="flex justify-between items-center text-sm">
                                        <div class="flex flex-col"><span class="font-bold text-gray-900 uppercase tracking-widest text-[10px]" x-text="option.name"></span><span class="text-[10px] text-gray-500 uppercase tracking-wide">Entrega em até <span x-text="option.days"></span> dias úteis</span></div>
                    ...
                    <div class="flex gap-2 justify-start mt-4">
                        <button @click="toggleWishlist()" :aria-pressed="isInWishlist.toString()" class="w-12 h-12 flex items-center justify-center border rounded-none transition-all cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black" :class="isInWishlist ? 'bg-red-50 border-red-200 text-red-600' : 'border-gray-200 text-gray-500 hover:border-black hover:text-black'">
                            <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" :fill="isInWishlist ? 'currentColor' : 'none'" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" /></svg>
                        </button>
                        <button @click="copyLink()" class="w-12 h-12 flex items-center justify-center border border-gray-200 rounded-none text-gray-500 hover:border-black hover:text-black transition-all cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black">
        ...
        {{-- ACORDEONS (INFORMAÇÕES INFERIORES) --}}
        <div class="border-t border-gray-200 mt-16 max-w-4xl mx-auto">
            <div x-data="{ open: true }" class="border-b border-gray-200">
                <button @click="open = !open" class="flex justify-between items-center w-full py-6 text-left focus:outline-none focus:ring-2 focus:ring-black group cursor-pointer">
            ...
            {{-- AVALIAÇÕES E GALERIA DE FOTOS (LIGHTBOX) --}}
            <div x-data="{ open: false }" class="border-b border-gray-200">
                <button @click="open = !open" class="flex justify-between items-center w-full py-6 text-left focus:outline-none focus:ring-2 focus:ring-black group cursor-pointer">
                    <span class="text-xl font-bold text-gray-900 uppercase tracking-wide group-hover:text-gray-600 transition">Avaliações {{ $reviewsCount > 0 ? "($reviewsCount)" : '' }}</span>
                    <span aria-hidden="true" x-text="open ? '-' : '+'" class="text-3xl font-light text-gray-400 group-hover:text-black transition"></span>
                </button>
                <div x-show="open" x-transition class="pb-8 text-gray-600">
                    @if($product->reviews && $product->reviews->count() > 0)
                        @foreach($product->reviews as $review) 
                            <div class="mb-6 border-b border-gray-100 pb-6 last:border-0 last:pb-0">
                                ...
                                <div class="flex text-black mb-2">@for($i=0; $i<$review->rating; $i++) <span aria-hidden="true">★</span> @endfor</div>
                                <p class="text-sm leading-relaxed text-gray-700">{{ $review->comment }}</p>
                                
                                {{-- ✏️ Correção: GALERIA DE AVALIAÇÃO COM BOTÕES SÓLIDOS, SEM OPACITY-0 E FECHAMENTO AO CLICAR FORA --}}
                                @if(!empty($review->images) && is_array($review->images))
                                    @php
                                        $galleryUrls = array_map(fn($img) => asset('storage/' . $img), $review->images);
                                    @endphp
                                    ...
                                    <div class="mt-4" x-data="{ 
                                        ...
                                        closeGallery() { this.isOpen = false; document.body.style.overflow = ''; },
                                        next() { this.currentIndex = (this.currentIndex + 1) % this.images.length; },
                                        prev() { this.currentIndex = this.currentIndex === 0 ? this.images.length - 1 : this.currentIndex - 1; }
                                    }">
                                        <div class="flex gap-2 flex-wrap">
                                            @foreach($review->images as $index => $reviewImg)
                                                <button type="button" @click="openGallery({{ $index }})" class="w-16 h-16 border border-gray-200 bg-white overflow-hidden p-1 transition-opacity hover:opacity-80 cursor-pointer focus:outline-none focus:ring-2 focus:ring-black">
                                                    <img src="{{ asset('storage/' . $reviewImg) }}" class="w-full h-full object-cover">
                                                </button>
                                            @endforeach
                                        </div>
                                        ...
                                        <template x-teleport="body">
                                            <div x-show="isOpen" style="display: none;" 
                                                 class="fixed inset-0 z-[9999] flex flex-col bg-black/80 backdrop-blur-md transition-opacity duration-300"
                                                 @keydown.escape.window="closeGallery()"
                                                 @keydown.right.window="next()"
                                                 @keydown.left.window="prev()"
                                                 @click.self="closeGallery()">
                                                
                                                <div class="w-full flex justify-end p-4 sm:p-6" @click.self="closeGallery()">
                                                    <button @click="closeGallery()" class="text-white hover:text-gray-300 text-4xl leading-none p-2 cursor-pointer focus:outline-none">&times;</button>
                                                </div>

                                                <div class="flex-1 w-full flex items-center justify-between px-2 sm:px-8 overflow-hidden" @click.self="closeGallery()">
                                                    <button x-show="images.length > 1" @click.stop="prev()" aria-label="Imagem anterior" class="flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 bg-white text-black hover:bg-black hover:text-white border border-gray-200 transition-colors duration-300 focus:outline-none shrink-0 cursor-pointer group rounded-none shadow-md z-10">
                                                        <svg class="w-8 h-8 sm:w-10 sm:h-10 group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="square" stroke-linejoin="miter" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
                                                    </button>

                                                    <div class="flex-1 h-full flex items-center justify-center p-2 sm:p-4" @click.self="closeGallery()">
                                                        <img :src="images[currentIndex]" class="max-w-full max-h-full object-contain shadow-2xl rounded-none" @click.stop="" draggable="false">
                                                    </div>
                                                    
                                                    <button x-show="images.length > 1" @click.stop="next()" aria-label="Próxima imagem" class="flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 bg-white text-black hover:bg-black hover:text-white border border-gray-200 transition-colors duration-300 focus:outline-none shrink-0 cursor-pointer group rounded-none shadow-md z-10">
                                                        <svg class="w-8 h-8 sm:w-10 sm:h-10 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="square" stroke-linejoin="miter" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
                                                    </button>
                                                </div>

                                                <div x-show="images.length > 1" class="w-full flex items-center justify-center gap-3 overflow-x-auto px-4 pb-8 pt-4 shrink-0" @click.self="closeGallery()">
                                                    <template x-for="(img, idx) in images" :key="idx">
                                                        <button @click.stop="currentIndex = idx"
                                                                class="w-16 h-16 border bg-white overflow-hidden transition-all duration-200 cursor-pointer focus:outline-none shrink-0"
                                                                :class="currentIndex === idx ? 'border-white ring-2 ring-white scale-110 opacity-100' : 'border-gray-500 opacity-50 hover:opacity-100'">
                        ...
                        @endforeach 
                    @else 
                        <p class="italic text-gray-500">Este produto ainda não possui nenhuma avaliação.</p> 
                    @endif
                </div>
            </div>
        </div>
                ...
                @foreach($relatedProducts as $related)
                    <div class="block group relative"
                         x-data="{ 
                              ...
                              async toggleFav() {
                                  if (!{{ auth()->check() ? 'true' : 'false' }}) { window.dispatchEvent(new CustomEvent('open-auth-slider')); return; }
                                  let previous = this.isFavorite; this.isFavorite = !this.isFavorite;
                                  try {
                                      const res = await fetch('/favoritos/toggle', {
                                          method: 'POST',
                                          headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=\'csrf-token\']')?.getAttribute('content') || '{{ csrf_token() }}', 'Accept': 'application/json' },
                                          body: JSON.stringify({ product_id: {{ $related->id }} })
                                      });
                                      if (!res.ok) throw new Error();
                                      const data = await res.json();
                                      if(data.success) { window.dispatchEvent(new CustomEvent('show-toast', { detail: { message: data.message } })); } else { throw new Error(); }
                                  } catch(e) {
                        ...
                        <div class="relative w-full border border-gray-200 mb-4 bg-white overflow-hidden" style="aspect-ratio: 1 / 1;">
                            <button type="button" class="absolute z-30 transition-colors duration-300 focus:outline-none bg-transparent border-none p-0 m-0 cursor-pointer pointer-events-auto"
                                    :class="isFavorite ? 'text-red-600 opacity-100' : 'text-gray-400 opacity-0 group-hover:opacity-100 hover:text-red-500'"
                                    style="top: 0.75rem; right: 0.75rem;" aria-label="Favoritar {{ $related->name }}" @click.stop.prevent="toggleFav()">
                                <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" :fill="isFavorite ? 'currentColor' : 'none'" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 hover:scale-110 transition-transform"><path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" /></svg>
                            </button>
                            ...
                            <a href="{{ route('shop.product', $related->slug) }}" class="absolute inset-0 block z-0 cursor-pointer">
                                @if($related->created_at->diffInDays(now()) < 30)
                                    <div class="absolute top-3 left-3 bg-black text-white text-[10px] font-bold px-3 py-1 uppercase tracking-widest z-10 shadow-sm pointer-events-none">Novo</div>
                                @endif
                                <img src="{{ Storage::url($related->image_url) }}" :src="currentImage" class="absolute inset-0 w-full h-full object-contain p-6 transition-transform duration-500" :class="hovering ? 'scale-105' : ''" loading="lazy">
                            </a>
                            ...
                            <div class="absolute bottom-0 left-0 w-full opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 z-20">
                                ...
                                @if($variantCount > 1)
                                    <a href="{{ route('shop.product', $related->slug) }}" class="w-full block bg-black text-white border-t border-gray-200 py-3 text-center uppercase font-bold text-xs tracking-widest hover:bg-white hover:text-black transition-colors duration-300 cursor-pointer">ADICIONAR AO CARRINHO</a>
                                @elseif($variantCount === 1)
                                    <form x-data="{ loading: false }" class="m-0 p-0" @submit.prevent="window.dispatchEvent(new CustomEvent('open-cart')); window.dispatchEvent(new CustomEvent('start-cart-loading')); loading = true; fetch('{{ route('cart.add', $related->id) }}', { method: 'POST', body: new FormData($event.target), headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } }).then(response => response.json()).then(data => { loading = false; if(data.success) { Livewire.dispatch('cartUpdated'); } else { window.dispatchEvent(new CustomEvent('update-cart-count')); alert(data.error || 'Erro ao adicionar ao carrinho'); } }).catch(error => { loading = false; window.dispatchEvent(new CustomEvent('update-cart-count')); alert('Ocorreu um erro de conexão.'); });">
                                        @csrf <input type="hidden" name="variant_id" value="{{ $related->variants->first()->id }}">
                                        <button type="submit" :disabled="loading" class="w-full bg-black text-white border-t border-gray-200 py-3 uppercase font-bold text-xs tracking-widest hover:bg-white hover:text-black transition-colors duration-300 flex items-center justify-center cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed focus:outline-none">
                                            <span x-show="!loading">Adicionar ao Carrinho</span>
                                            <span x-show="loading" style="display: none;"><svg class="animate-spin h-4 w-4 text-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></span>
                                        </button>
                                    </form>
                                @else
                                    <div class="w-full bg-gray-100 border-t border-gray-200 text-gray-400 py-3 uppercase font-bold text-xs tracking-widest flex items-center justify-center">Indisponível</div>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="text-left space-y-1">
                            ...
                            @endif
                            <a href="{{ route('shop.product', $related->slug) }}" class="block cursor-pointer"><h4 class="font-bold text-gray-900 line-clamp-1">{{ $related->name }}</h4></a>
                            <div class="mt-1 cursor-text">
                                @if($related->isOnSale())
                                    <div class="flex flex-col items-start justify-center gap-0.5"><span class="font-bold text-red-600 text-lg leading-tight">R$ {{ number_format($related->sale_price, 2, ',', '.') }}</span><div class="flex items-center gap-2"><span class="text-xs text-gray-400 line-through">R$ {{ number_format($related->base_price, 2, ',', '.') }}</span><span class="bg-red-100 text-red-800 text-[10px] font-bold px-1.5 py-0.5 rounded">-{{ $related->discount_percentage }}%</span></div></div>
                                @else
                            ...
                            @if($related->variants->whereNotNull('image')->count() > 0)
                                <div class="flex justify-start gap-2 pt-2 flex-wrap">
                                    @foreach($related->visual_variants as $variant)
                                        <div @mouseenter="currentImage = '{{ Storage::url($variant->image) }}'" @mouseleave="currentImage = originalImage" @click.stop.prevent="window.location.href = '{{ route('shop.product', $related->slug) }}?variant={{ $variant->id }}'" class="w-8 h-8 rounded-none border border-gray-300 shadow-sm overflow-hidden cursor-pointer bg-white hover:border-black transition-all flex items-center justify-center">
                                            <img src="{{ Storage::url($variant->image) }}" class="w-full h-full object-contain p-0.5 pointer-events-none" loading="lazy">
                                        </div>
                                    @endforeach
                                </div>
                            @endif
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('productSelector', (config) => ({
                loading: false, variants: config.variants, optionsMap: config.optionsMap, selectedOptions: {}, cartVariant: null, displayVariant: config.initialVariant, promoText: config.initialPromoText, colorImages: config.colorImages, currentImage: config.baseImages[0], galleryImages: config.baseImages, timerDisplay: '', timerInterval: null, showError: false, errorMessage: '', isInWishlist: config.isInWishlist, copied: false,
                init() {
                    if (this.displayVariant) {
                        if (this.displayVariant.gallery) { this.galleryImages = this.displayVariant.gallery; this.currentImage = this.galleryImages[0]; }
                        if (this.displayVariant.options) { this.selectedOptions = { ...this.displayVariant.options }; this.checkCartReadiness(); }
                    }
                    if (Object.keys(this.optionsMap).length === 0 && this.variants.length > 0) { this.cartVariant = this.variants[0]; this.displayVariant = this.cartVariant; }
                    this.updateTimerFromDisplay();
                },
                async toggleWishlist() {
                    if (!{{ auth()->check() ? 'true' : 'false' }}) { window.dispatchEvent(new CustomEvent('open-auth-slider')); return; }
                    const previousState = this.isInWishlist; this.isInWishlist = !this.isInWishlist; 
                    try {
                        const response = await fetch('/favoritos/toggle', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '{{ csrf_token() }}', 'Accept': 'application/json' }, body: JSON.stringify({ product_id: {{ $product->id }} }) });
                        if(!response.ok) throw new Error(); const data = await response.json();
                        if (data.success) { window.dispatchEvent(new CustomEvent('show-toast', { detail: { message: data.message } })); } else { throw new Error(); }
                    } catch (e) { this.isInWishlist = previousState; window.dispatchEvent(new CustomEvent('show-toast', { detail: { message: 'Erro ao atualizar favoritos.', type: 'error' } })); }
                },
                copyLink() { navigator.clipboard.writeText(window.location.href); this.copied = true; setTimeout(() => this.copied = false, 2000); },
                selectOption(key, value) { this.selectedOptions[key] = value; this.showError = false; if (this.colorImages[value]) { this.galleryImages = this.colorImages[value]; this.currentImage = this.galleryImages[0]; } this.updateDisplayData(); this.checkCartReadiness(); },
                updateDisplayData() {
                    let candidates = this.variants.filter(v => { for (const [k, val] of Object.entries(this.selectedOptions)) { if (v.options[k] !== val) return false; } return true; });
                    if (candidates.length > 0) {
                        candidates.sort((a, b) => { if (a.is_on_sale && !b.is_on_sale) return -1; if (!a.is_on_sale && b.is_on_sale) return 1; let pA = a.is_on_sale ? (a.sale_price || a.price) : a.price; let pB = b.is_on_sale ? (b.sale_price || b.price) : b.price; return pA - pB; });
                        this.displayVariant = candidates[0]; this.updateTimerFromDisplay();
                    }
                },
                checkCartReadiness() { const requiredKeys = Object.keys(this.optionsMap); const selectedKeys = Object.keys(this.selectedOptions); if (requiredKeys.length !== selectedKeys.length) { this.cartVariant = null; return; } const exactMatch = this.variants.find(v => { for (const [k, val] of Object.entries(this.selectedOptions)) { if (v.options[k] !== val) return false; } return true; }); if (exactMatch) this.cartVariant = exactMatch; else this.cartVariant = null; },
                updateTimerFromDisplay() { this.stopTimer(); if (this.displayVariant && this.displayVariant.is_on_sale && this.displayVariant.sale_end_date) { this.startTimer(this.displayVariant.sale_end_date); } },
                ...
                async submitCart(form) { 
                    if (this.cartVariant && this.cartVariant.stock > 0) { 
                        this.loading = true; window.dispatchEvent(new CustomEvent('open-cart')); window.dispatchEvent(new CustomEvent('start-cart-loading'));
                        try { const response = await fetch(form.action, { method: 'POST', body: new FormData(form), headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } }); const data = await response.json(); this.loading = false; if (data.success) { Livewire.dispatch('cartUpdated'); } else { window.dispatchEvent(new CustomEvent('update-cart-count')); alert(data.error || 'Erro ao adicionar ao carrinho'); } } catch (error) { this.loading = false; window.dispatchEvent(new CustomEvent('update-cart-count')); console.error('Erro:', error); alert('Ocorreu um erro de conexão.'); }
                    } else { this.showError = true; if (this.cartVariant && this.cartVariant.stock <= 0) this.errorMessage = "Produto esgotado."; else { const allOptions = Object.keys(this.optionsMap); const selected = Object.keys(this.selectedOptions); const missing = allOptions.filter(opt => !selected.includes(opt)); if (missing.length > 0) this.errorMessage = "Por favor, selecione: " + missing.join(', '); else this.errorMessage = "Opção indisponível."; } }
                },
                startTimer(date) { const end = new Date(date).getTime(); this.calcTime(end); this.timerInterval = setInterval(() => { this.calcTime(end); }, 1000); },
                calcTime(end) { const now = new Date().getTime(); const d = end - now; if(d < 0) { this.timerDisplay = "EXPIRADO"; this.stopTimer(); return; } const days = Math.floor(d / (1000 * 60 * 60 * 24)); const hours = Math.floor((d % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)); const minutes = Math.floor((d % (1000 * 60 * 60)) / (1000 * 60)); const seconds = Math.floor((d % (1000 * 60)) / 1000); this.timerDisplay = `${days}d ${hours}h ${minutes}m ${seconds}s`; },
