        ...
        @if(isset($items) && $items->count() > 0)
            <div class="bg-white shadow-sm border border-gray-100 rounded-xl overflow-hidden">
                            ...
                            <tr class="group hover:bg-gray-50/50 transition">
                                ...
                                <td class="p-4 text-right">
                                    <form action="{{ route('cart.remove', $item->id) }}" method="POST">
                                        @csrf @method('DELETE')
                                        <button class="text-red-400 hover:text-red-600 transition-colors" title="Remover item">
                ...
                <div class="bg-gray-50 p-6 flex flex-col md:flex-row justify-between items-center border-t border-gray-100 gap-4">
                    ...
                    @auth
                        <a href="{{ url('/checkout') }}" class="w-full md:w-auto bg-black text-white px-8 py-4 rounded-xl font-bold uppercase tracking-widest hover:bg-gray-800 transition shadow-lg text-center">
                            Finalizar Compra
                        </a>
                    @else
                        <a href="{{ route('login') }}" class="w-full md:w-auto bg-black text-white px-8 py-4 rounded-xl font-bold uppercase tracking-widest hover:bg-gray-800 transition shadow-lg text-center">
                            Login para Comprar
                        </a>
                    @endauth
                </div>
