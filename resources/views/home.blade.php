<x-layout>
            ...
            @foreach($newArrivals as $product)
                ...
                <a href="{{ route('shop.product', $product->slug) }}" class="block group cursor-pointer">
                    ...
                    <div class="text-center space-y-1">
                      ...
                      {{-- SUBSTITUA O PREÇO ANTIGO POR ESTE BLOCO --}}
                      <div class="mt-1">
    @if($product->isOnSale())
        <div class="flex flex-col items-center justify-center gap-0.5">
            {{-- 1. Preço da Oferta (EM CIMA, Maior e Vermelho) --}}
    ...
    @foreach($collections as $collection)
        <section class="mb-24">
            {{-- BANNER DA COLEÇÃO --}}
            <div class="relative w-full h-[450px] md:h-[600px] mb-12 group overflow-hidden">
                 ...
                 <div class="absolute bottom-0 left-0 p-8 md:p-12 w-full md:max-w-4xl flex flex-col justify-end items-start text-left z-10">
                     <h2 class="text-5xl font-black uppercase mb-2 tracking-tighter drop-shadow-md text-white">
                        {{ $collection->title }}
                     </h2>
                     @if($collection->description)
                        <p class="text-xl text-white/90 font-medium drop-shadow-sm max-w-2xl">
