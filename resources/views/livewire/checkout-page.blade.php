<div class="min-h-screen pt-32 pb-10"
    ...
    <div class="container mx-auto px-4 max-w-7xl">
        ...
        @if (session()->has('error'))
            <div class="bg-white border border-red-500 text-red-700 px-4 py-3 rounded-none relative mb-6">
        ...
        <form x-data="paymentManager()" @submit.prevent="submitOrder" class="flex flex-col lg:flex-row gap-8 relative">
            ...
            <div class="order-2 lg:order-1 w-full lg:w-2/3 space-y-6">
                ...
                <section class="bg-white p-6 rounded-none border border-gray-200">
                    <h2 class="text-lg font-black text-gray-900 mb-4 flex items-center gap-2 uppercase tracking-tight">
                ...
                <section class="bg-white p-6 rounded-none border border-gray-200 relative">
                    <div wire:loading wire:target="selectedAddressId, useNewAddress" class="absolute inset-0 z-10 bg-white/50 backdrop-blur-[1px] rounded-none"></div>
                    <h2 class="text-lg font-black text-gray-900 mb-4 flex items-center gap-2 uppercase tracking-tight">
                        ...
                        <div class="space-y-3 mb-4">
                            @foreach(Auth::user()->addresses as $addr)
                                <label :class="addrId == {{ $addr->id }} && !newAddr ? 'border-black bg-white ring-1 ring-black' : 'border-gray-200 hover:border-black bg-white'" 
                                       class="flex items-start p-4 border rounded-none cursor-pointer transition-colors">
                                    ...
                                    <div class="ml-3">
                                        ...
                                        <p class="text-xs text-gray-500 mt-1 font-mono">CEP: {{ $addr->zip_code }}</p>
                                    </div>
                                </label>
                            @endforeach
                            
                            <label :class="newAddr ? 'border-black bg-white ring-1 ring-black' : 'border-gray-200 hover:border-black bg-white'" 
                                   class="flex items-center p-4 border rounded-none cursor-pointer transition-colors">
                    ...
                    <div x-show="newAddr || {{ Auth::user()->addresses->isEmpty() ? 'true' : 'false' }}" x-transition style="display: none;" class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-gray-200">
                        <div class="col-span-1 md:col-span-2">
                            <label class="block text-xs font-bold text-gray-700 mb-1 uppercase tracking-widest">CEP</label>
                            <input type="text" wire:model.live.debounce.500ms="newAddress.zip_code" x-on:input="$el.value = formatCEP($el.value)" maxlength="9" class="appearance-none rounded-none block w-full px-3 py-3 border {{ $errors->has('newAddress.zip_code') ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-black focus:border-black' }} bg-white placeholder-gray-400 text-gray-900 focus:outline-none sm:text-sm" placeholder="00000-000">
                            @error('newAddress.zip_code') <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-span-1 md:col-span-2">
                            <label class="block text-xs font-bold text-gray-700 mb-1 uppercase tracking-widest">Rua / Avenida</label>
                            <input type="text" wire:model="newAddress.street" class="appearance-none rounded-none block w-full px-3 py-3 border {{ $errors->has('newAddress.street') ? 'border-red-500' : 'border-gray-300' }} bg-white placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-black focus:border-black sm:text-sm">
                            @error('newAddress.street') <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1 uppercase tracking-widest">Número</label>
                            <input type="text" wire:model="newAddress.number" class="appearance-none rounded-none block w-full px-3 py-3 border {{ $errors->has('newAddress.number') ? 'border-red-500' : 'border-gray-300' }} bg-white placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-black focus:border-black sm:text-sm">
                            @error('newAddress.number') <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1 uppercase tracking-widest">Complemento</label>
                            <input type="text" wire:model="newAddress.complement" class="appearance-none rounded-none block w-full px-3 py-3 border border-gray-300 bg-white placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-black focus:border-black sm:text-sm" placeholder="Apto, Bloco">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1 uppercase tracking-widest">Bairro</label>
                            <input type="text" wire:model="newAddress.neighborhood" class="appearance-none rounded-none block w-full px-3 py-3 border {{ $errors->has('newAddress.neighborhood') ? 'border-red-500' : 'border-gray-300' }} bg-white placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-black focus:border-black sm:text-sm">
                            @error('newAddress.neighborhood') <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1 uppercase tracking-widest">Cidade</label>
                            <input type="text" wire:model="newAddress.city" class="appearance-none rounded-none block w-full px-3 py-3 border {{ $errors->has('newAddress.city') ? 'border-red-500' : 'border-gray-300' }} bg-white placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-black focus:border-black sm:text-sm">
                            @error('newAddress.city') <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-span-1 md:col-span-2">
                            <label class="block text-xs font-bold text-gray-700 mb-1 uppercase tracking-widest">Estado (UF)</label>
                            <input type="text" wire:model="newAddress.state" maxlength="2" class="appearance-none rounded-none block w-full px-3 py-3 border {{ $errors->has('newAddress.state') ? 'border-red-500' : 'border-gray-300' }} bg-white placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-black focus:border-black sm:text-sm uppercase" placeholder="SP">
                            @error('newAddress.state') <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                        </div>
                    </div>
                </section>

               <section wire:init="loadInitialShipping" class="bg-white p-6 rounded-none border border-gray-200">
                    ...
                    <div class="relative min-h-[100px] w-full">
                        ...
                        <div wire:loading.class="opacity-20 pointer-events-none blur-[2px]" wire:target="newAddress.zip_code, selectedAddressId, useNewAddress, loadInitialShipping" class="transition-all duration-300 w-full">
                                        ...
                                        <label :class="shipMethod == '{{ $option['id'] }}' ? 'border-black bg-white ring-1 ring-black' : 'border-gray-200 hover:border-black bg-white'" 
                                            ...
                                            <div class="ml-3 w-full flex justify-between items-center">
                                                <div class="pr-2">
                                                    ...
                                                    <p class="text-xs text-gray-500 uppercase tracking-wide">Em até {{ $option['days'] }} dias úteis</p>
                                                </div>
                            ...
                            @else
                                <div class="text-xs font-bold uppercase tracking-widest text-gray-500 bg-white p-6 rounded-none text-center border border-gray-200 flex items-center justify-center h-full w-full">
                ...
                <section class="bg-white p-6 rounded-none border border-gray-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-4">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1 uppercase tracking-widest">Nome</label>
                            <input type="text" wire:model="firstName" class="appearance-none rounded-none block w-full px-3 py-3 border {{ $errors->has('firstName') ? 'border-red-500' : 'border-gray-300' }} bg-white placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-black focus:border-black sm:text-sm">
                            @error('firstName') <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1 uppercase tracking-widest">Sobrenome</label>
                            <input type="text" wire:model="lastName" class="appearance-none rounded-none block w-full px-3 py-3 border {{ $errors->has('lastName') ? 'border-red-500' : 'border-gray-300' }} bg-white placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-black focus:border-black sm:text-sm">
                            @error('lastName') <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1 uppercase tracking-widest">CPF</label>
                            <input type="text" wire:model="cpf" x-on:input="$el.value = formatCPF($el.value); $wire.set('cpf', $el.value)" maxlength="14" class="appearance-none rounded-none block w-full px-3 py-3 border {{ $errors->has('cpf') ? 'border-red-500' : 'border-gray-300' }} bg-white placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-black focus:border-black sm:text-sm" placeholder="000.000.000-00">
                            @error('cpf') <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1 uppercase tracking-widest">Telefone / WhatsApp</label>
                            <input type="text" wire:model="phone" x-on:input="$el.value = formatPhone($el.value); $wire.set('phone', $el.value)" maxlength="15" class="appearance-none rounded-none block w-full px-3 py-3 border {{ $errors->has('phone') ? 'border-red-500' : 'border-gray-300' }} bg-white placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-black focus:border-black sm:text-sm" placeholder="(11) 99999-9999">
                ...
                <section class="bg-white p-6 rounded-none border border-gray-200">
                    <h2 class="text-lg font-black text-gray-900 mb-4 flex items-center gap-2 uppercase tracking-tight">
                        <span class="bg-black text-white rounded-none w-6 h-6 flex items-center justify-center text-sm">5</span> 
                        Cupom de Desconto
                    </h2>
                    <div class="flex gap-2">
                        ...
                        <button type="button" wire:click="applyCoupon" class="bg-black text-white border border-black px-6 rounded-none font-bold hover:bg-white hover:text-black transition duration-300 text-sm uppercase tracking-widest whitespace-nowrap cursor-pointer">
                            Aplicar
                        </button>
                    </div>
                    @error('couponCode') 
                    ...
                    @if (session()->has('coupon_success')) 
                        <p class="text-green-600 text-xs mt-2 font-bold uppercase tracking-widest">{{ session('coupon_success') }}</p> 
                    @endif
                </section>

                <section class="bg-white p-6 rounded-none border border-gray-200">
                    <h2 class="text-lg font-black text-gray-900 mb-4 flex items-center gap-2 uppercase tracking-tight">
                        <span class="bg-black text-white rounded-none w-6 h-6 flex items-center justify-center text-sm">6</span> 
                        Forma de Pagamento
                    </h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <label :class="payMethod === 'credit_card' ? 'border-black bg-white ring-1 ring-black' : 'border-gray-200 hover:border-black bg-white'" 
                               class="flex flex-col items-center justify-center p-4 border rounded-none cursor-pointer transition-colors text-center h-24">
                        ...
                        <label :class="payMethod === 'boleto' ? 'border-black bg-white ring-1 ring-black' : 'border-gray-200 hover:border-black bg-white'" 
                            ...
                            <span class="font-bold text-xs uppercase tracking-widest">Boleto</span>
                        </label>
                    </div>
                ...
                {{-- Resumo Mobile --}}
                <div class="bg-white p-6 rounded-none border border-gray-200 lg:hidden mb-6">
                    <div wire:loading wire:target="shippingMethod" class="w-full h-full absolute inset-0 bg-white/50 backdrop-blur-[1px] z-10 rounded-none"></div>
                    ...
                    <div class="space-y-3 relative z-0">
                        ...
                        <div class="flex justify-between text-sm text-gray-600">
                            <span class="uppercase tracking-widest text-xs font-bold">Frete</span>
            ...
            <div class="order-1 lg:order-2 w-full lg:w-1/3">
                <div class="bg-white p-6 rounded-none border border-gray-200 sticky top-24 relative">
                    ...
                    <div wire:loading wire:target="shippingMethod" class="absolute inset-0 z-10 flex items-center justify-center bg-white/50 backdrop-blur-[1px] rounded-none"></div>
                    ...
                    <div class="space-y-4 mb-6 pr-2 relative z-0">
                        @foreach($this->cartItems as $item)
                            ...
                            <div class="flex gap-4">
                                ...
                                <div class="w-16 h-20 bg-white rounded-none overflow-hidden flex-shrink-0 border border-gray-200 flex items-center justify-center">
                                    <img src="{{ Storage::url($img) }}" alt="{{ $item->product->name }}" class="w-full h-full object-contain p-1">
                                </div>
                                        ...
                                        @if($item->variant && is_array($item->variant->options) && count($item->variant->options) > 0)
                                            <div class="flex flex-wrap gap-x-3 text-[10px] uppercase tracking-widest text-gray-500 mt-1">
                                                @foreach($item->variant->options as $key => $value)
                                                    <span><strong class="font-semibold text-gray-700">{{ $key }}:</strong> {{ $value }}</span>
                                                @endforeach
                                            </div>
                                        @endif
                    ...
                    <div class="border-t border-gray-200 pt-4 space-y-3">
                        ...
                        @if($discount > 0.01)
                            <div class="flex justify-between text-sm text-emerald-600 font-bold">
                                <span class="uppercase tracking-widest text-xs">Cupom {{ $couponDisplay }}</span>
                                <span>- R$ {{ number_format($discount, 2, ',', '.') }}</span>
                            </div>
                        @endif
