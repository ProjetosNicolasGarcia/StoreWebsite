...
<body class="bg-white text-gray-900 font-sans antialiased">
    ...
    >
        ...
        <div class="container mx-auto px-2 sm:px-4 lg:px-8 grid grid-cols-3 items-center relative">
                                ...
                                <div class="grid grid-cols-2 gap-x-8 gap-y-2">
                                    @if(isset($globalCategories) && $globalCategories->count() > 0)
                                        @foreach($globalCategories as $category)
                                            <a href="#" class="block !text-gray-600 hover:!text-red-600 font-medium text-sm transition-colors py-1">
                                                {{ $category->name }}
                                            </a>
                                        @endforeach
                                    @else
            ...
            {{-- ALTERAÇÃO: 'text-lg' no mobile e 'tracking-tight' para economizar espaço horizontal --}}
            ...
            {{-- ALTERAÇÃO: 'space-x-1' no mobile para juntar os ícones --}}
                    ...
                    <nav class="flex flex-col space-y-6">
                        <div x-data="{ expanded: false }">
                            ...
                            <div x-show="expanded" x-collapse class="mt-4 pl-4 flex flex-col space-y-3 text-base">
                                @if(isset($globalCategories) && $globalCategories->count() > 0)
                                    @foreach($globalCategories as $category)
                                        <a href="#" class="block !text-black font-medium hover:!text-red-600">{{ $category->name }}</a>
                                    @endforeach
                                @else
    ...
    <footer class="bg-gray-100 text-gray-600 py-16 mt-20 text-sm border-t border-gray-200">
        <div class="container mx-auto px-8 grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          ...
          {{-- Instagram (Versão Sólida/Preenchida) --}}
<a href="#" class="text-gray-500 hover:text-pink-600 transition transform hover:-translate-y-1" title="Instagram">
    <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zm0 10.324a4.162 4.162 0 1 1 0-8.324 4.162 4.162 0 0 1 0 8.324zm6.406-11.845a1.44 1.44 0 1 0 0 2.88 1.44 1.44 0 0 0 0-2.88z" />
    </svg>
</a>
                    {{-- Twitter (X) --}}
