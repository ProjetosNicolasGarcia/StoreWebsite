...
<div 
    x-data="{ 
        ...
        async submitLogin() {
            ...
            this.errors = {};
            try {
                const res = await axios.post('{{ route('store.login') }}', { email: this.email, password: this.password });
                ...
                if(e.response && e.response.status === 422) {
                    ...
                    alert('Erro no Sistema: ' + (e.response?.data?.message || e.message));
                }
            } finally {
                this.loading = false;
            }
        ...
        async submit2FA() {
            ...
            this.errors = {};
            try {
                await axios.post('{{ route('store.verify') }}', { email: this.email, code: this.code });
                window.location.reload();
            } catch (e) {
                if(e.response && e.response.status === 422) {
                    this.errors = e.response.data.errors;
        ...
        async submitRegister() {
            ...
            this.errors = {};
            try {
                const res = await axios.post('{{ route('store.register') }}', {
                ...
                if(e.response && e.response.status === 422) {
                    ...
                    alert('Erro ao Criar Conta: ' + (e.response?.data?.message || e.message));
                }
            } finally {
                this.loading = false;
            }
        ...
        <div class="p-6">
            ...
            <div x-show="mode === 'login'">
                <form @submit.prevent="submitLogin" class="space-y-5">
                    <div>
                        ...
                        {{-- ALTERADO: bg-white e border-gray-500 --}}
                    ...
                    <div class="flex items-center justify-end">
                        <button type="button" @click="mode = 'forgot'" class="text-sm text-gray-600 hover:text-black hover:underline cursor-pointer">Esqueceu a senha?</button>
                    </div>

                    <button type="submit" :disabled="loading" 
                            class="w-full flex justify-center items-center h-12 px-4 border border-black rounded-xl shadow-sm text-base font-bold text-white bg-black hover:bg-white hover:text-black transition-all duration-200 cursor-pointer focus:outline-none">
                ...
                <div class="mt-8">
                    ...
                    <div class="mt-8">
                        <a href="{{ route('login.google') }}" 
                           class="w-full flex justify-center items-center gap-2 h-12 px-4 border border-gray-300 rounded-xl shadow-sm bg-white text-sm font-medium text-gray-700 hover:border-black hover:text-black transition-all cursor-pointer">
            ...
            <div x-show="mode === '2fa'" class="space-y-5">
                ...
                <div>
                    ...
                    <p x-show="errors.code" class="text-red-500 text-xs mt-1" x-text="errors.code"></p>
                </div>
                <button @click="submit2FA" :disabled="loading" 
                        class="w-full h-12 px-4 border border-black rounded-xl text-base font-bold text-white bg-black hover:bg-white hover:text-black transition-all cursor-pointer">
                    VALIDAR
                </button>
                ...
                <form @submit.prevent="submitRegister" class="space-y-4">
                   ...
                   <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input :type="showPassword ? 'text' : 'password'" x-model="password_confirmation" 
                                   class="block w-full h-12 rounded-xl border border-gray-500 bg-white text-gray-900 shadow-none focus:border-black focus:ring-black transition-all pr-10">
                        </div>
                    </div>
                    
                    <p x-show="Object.keys(errors).length > 0" class="text-red-500 text-xs mt-2">
                        Verifique os campos acima.
                        <span x-text="Object.values(errors).flat().join(', ')"></span>
                    </p>

                    <button type="submit" :disabled="loading" 
                            class="w-full h-12 px-4 border border-black rounded-xl text-base font-bold text-white bg-black hover:bg-white hover:text-black transition-all cursor-pointer mt-4">
                        CRIAR CONTA
                    </button>
                </form>
            ...
            <div x-show="mode === 'forgot'" class="space-y-5">
                ...
                <form action="{{ route('password.email') }}" method="POST">
                    ...
                    <button type="submit" class="w-full h-12 px-4 border border-black rounded-xl text-base font-bold text-white bg-black hover:bg-white hover:text-black transition-all cursor-pointer">
                        ENVIAR LINK
                    </button>
                </form>
                <button @click="mode = 'login'" class="w-full mt-2 text-sm text-gray-500 hover:text-black cursor-pointer">Voltar</button>
